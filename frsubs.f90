...
!
!   PURPOSE             - NUCLEUS CALLED ONLY BY IMSL ROUTINE EIGRF
!
...
!                                  REDUCE NORM A BY DIAGONAL SIMILARITY
!                                  TRANSFORMATION STORED IN D
!                                  FIRST EXECUTABLE STATEMENT
      L1 = 1
      K1 = N
!                                  SEARCH FOR ROWS ISOLATING AN EIGEN-
!                                    VALUE AND PUSH THEM DOWN
    5 K1P1 = K1+1
      IF (K1.LT.1) GO TO 35
      K11=K1
      DO 30 JJ=1,K11
...
!                                  NOW BALANCE
  ...
  120 IF (NOCONV.EQ.1) GO TO 75
      RETURN
      END SUBROUTINE EBALAF

!***** SUBROUTINE EBBCKF (D,Z,K,L,MM,N,IZ)
!   IMSL ROUTINE NAME   - EBBCKF
!
!-----------------------------------------------------------------------
!
!   COMPUTER            - CRAY/SINGLE
...
!                                  FIRST EXECUTABLE STATEMENT
    ...
    5    CONTINUE
   10 CONTINUE
!                                  INTERCHANGE ROWS IF PERMUTATIONS
!                                    OCCURRED IN EBALAF
   ...
   40 CONTINUE
   45 RETURN
      END SUBROUTINE EBBCKF

!***** SUBROUTINE EHBCKF (Z,H,D,N,MM,IZH,K,L)
!   IMSL ROUTINE NAME   - EHBCKF
!
!-----------------------------------------------------------------------
!
!   COMPUTER            - CRAY/SINGLE
...
!                                  FIRST EXECUTABLE STATEMENT
   ...
   30 CONTINUE
 9005 RETURN
      END SUBROUTINE EHBCKF

!***** SUBROUTINE EHESSF (A,K,L,N,IA,D)
!   IMSL ROUTINE NAME   - EHESSF
!
!-----------------------------------------------------------------------
!
!   COMPUTER            - CRAY/SINGLE
...
!                                  DO 30 J=L,M,-1
   ...
   45 CONTINUE
   50 RETURN
      END SUBROUTINE EHESSF
!
!
!***** SUBROUTINE EQRH3F (H,N,IH,K,L,WR,WI,Z,IZ,IER)
!   IMSL ROUTINE NAME   - EQRH3F
!
!-----------------------------------------------------------------------
!
!   COMPUTER            - CRAY/SINGLE
...
!                                  FIRST EXECUTABLE STATEMENT
      IER = 0
!                                  STORE ROOTS ISOLATED BY EBALAF
      RNORM = 0.0
      KA = 1
      DO 10 I=1,N
         DO 5 J=KA,N
...
!                                  END REAL VECTOR
         GO TO 220
!                                  LAST VECTOR COMPONENT CHOSEN
!                                    IMAGINARY SO THAT EIGENVECTOR
...
!                                  SET ERROR INDICATOR  TO THE INDEX
!                                  OF THE CURRENT EIGENVALUE
  250 IER = 128+IEN
      DO 255 I=1,IEN
         WR(I) = ZERO
         WI(I) = ZERO
 ...
 9005 RETURN
      END SUBROUTINE EQRH3F

!
!
!***** SUBROUTINE EIGRF  (A,N,IA,IJOB,W,Z,IZ,WK,IER)
!   IMSL ROUTINE NAME   - EIGRF
!
!-----------------------------------------------------------------------
!
!   COMPUTER            - CRAY/SINGLE
...
!
!   USAGE               - CALL EIGRF (A,N,IA,IJOB,W,Z,IZ,WK,IER)
!
!   ARGUMENTS    A      - THE INPUT REAL GENERAL MATRIX OF ORDER N
...
!                IA     - THE INPUT ROW DIMENSION OF MATRIX A EXACTLY
...
!                           GREATER THAN 100).
!                W      - THE OUTPUT COMPLEX VECTOR OF LENGTH N,
!                           CONTAINING THE EIGENVALUES OF A.
!                         NOTE - THE ROUTINE TREATS W AS A REAL VECTOR
!                           OF LENGTH 2*N. AN APPROPRIATE EQUIVALENCE
...
!                           STATEMENT MAY BE REQUIRED. SEE DOCUMENT
!                           EXAMPLE.
!                IZ     - THE INPUT ROW DIMENSION OF MATRIX Z EXACTLY
!                           AS SPECIFIED IN THE DIMENSION STATEMENT IN
!                           THE CALLING PROGRAM. IZ MUST BE GREATER
...
!                           IJOB = 1, THE LENGTH OF WK IS AT LEAST 2N.
!                           IJOB = 2, THE LENGTH OF WK IS AT LEAST
!                             (2+N)N.
!                           IJOB = 3, THE LENGTH OF WK IS AT LEAST 1.
!                IER    - ERROR PARAMETER. (OUTPUT)
!                         TERMINAL ERROR
!                           IER = 128+J, INDICATES THAT EQRH3F FAILED
!                           TO CONVERGE ON EIGENVALUE J. EIGENVALUES
!                           J+1,J+2,...,N HAVE BEEN COMPUTED CORRECTLY.
!                           EIGENVALUES 1,...,J ARE SET TO ZERO.
!                           IF IJOB = 1 OR 2 EIGENVECTORS ARE SET TO
...
!                       - SINGLE/H36,H48,H60
!
!   REQD. IMSL ROUTINES - EBALAF,EBBCKF,EHBCKF,EHESSF,EQRH3F,UERTST1,
!                           UGETIO1
!
!   NOTATION            - INFORMATION ON SPECIAL NOTATION AND
!                           CONVENTIONS IS AVAILABLE IN THE MANUAL
...
!                                  IF L = 0, A IS ALREADY IN HESSENBERG
!                                    FORM
      CALL EHESSF (A,K,L,N,N,WK(1,N2))
      IF (IJOB .EQ. 0) GO TO 35
!                                  SET Z IDENTITY MATRIX
      II = 1
      JJ = 1
      NP1 = N+1
      DO 30 I=1,N
...
!                                  CONVERT W (EIGENVALUES) TO COMPLEX
!                                    FORMAT
   40 DO 45 I=1,N
         NPI = N+I
         WK(I,N1) = W(NPI)
...
!                                    NEXT, MOVE ORIGINAL MATRIX BACK
!                                    TO A
...
!                                  COMPUTE PERFORMANCE INDEX
 ...
 9005 RETURN
      END SUBROUTINE EIGRF

      subroutine leqt1fl(a, m, n, ia, b, idgt, wkarea, ier)
      implicit none 
      save
!
!-leqt1f--------s/d-----library 2--------------------------------------
!
!   function            - linear equation solution - full storage
!                           mode - space economizer solution.
!   usage               - call leqt1f (a,m,n,ia,b,idgt,wkarea,ier)
!   parameters   a      - input matrix of dimension n by n containing
...
!                                 idgt is greater than 0 on input.
!                                 see chapter l prelude for further
!                                 discussion.
!   precision           - single/double
!   req'd IMSL routines - ludatf1,luelmf1,uertst1
!   language            - fortran
! ----------------------------------------------------------------------
!   latest revision     - march 22,1974
!                                  dec
!
...
!
      ier = 0
!                                  decompose a
!-YuP call ludatf1 (a,a,n,ia,idgt,d1,d2,wkarea,wkarea,wa,ier)
      call ludatf1 (a,a,n,ia,idgt,d1,d2,ipvt,wkarea,wa,ier) ! YuP: ipvt - out
      if (ier .gt. 128)  go to 9000
!                                  call routine luelmf1 (forward and
!                                  backward substitutions)
      do 10 j=1,m
!-YuP    call luelmf1 (a,b(1,j),wkarea,n,ia,b(1,j))
...
!
      ...
      implicit none 
!
!-ludatf1--------s/d-----library 2--------------------------------------
!
!   function            - l-u decomposition by the crout algorithm
!                           with optional accuracy test.
!   usage               - call ludatf1 (a,lu,n,ia,idgt,d1,d2,ipvt,
!                                       equil,wa,ier)
!   parameters   a      - input matrix of dimension n by n containing
...
!                           matrix a.
!                ia     - input scalar containing the row dimension of
...
!                                 discussion.
!   precision           - single/double
!   req'd IMSL routines - uertst1
!   language            - fortran
! ----------------------------------------------------------------------
!
!   latest revision     - march 22,1974 (dec)
!
      real*8 a(ia,*), equil(*)
      real*8   lu(ia,*)
      integer  ipvt(*)
...
!
 ...
 9005 return
!
      end subroutine ludatf1

      subroutine luelmf1 (a, b, ipvt, n, ia, x)
      implicit none 
!
!
!-luelmf1--------s/d-----library 2--------------------------------------
!
!   function            - elimination part of solution of ax = b -
!                           full storage mode
!   usage               - call luelmf1 (a,b,ipvt,n,ia,x)
!   parameters   a      - the result, lu, computed in the subroutine
!                           'ludatf1', where l is a lower triangular
...
!                           subroutine 'ludatf1', stored as an n length
!                           vector
...
!
      ...
      save
!
! --- changed name from UERTST to UERTST1 so that only IMSL routines
! --- with sources in ONETWO will call UERTST1. Other IMSL routines used
! --- in ONETWO which are linked from the IMSL library will thus not
! --- be involved with this subroutine.
!
! --- uertst1 ---------------- library 2 -------------------------------
!
!   function            - error message generation
!   usage               - call uertst1 (ier, obsolete)
...
!              obsolete - input scalar (double precision on dec)
!                         containing the name of the calling routine
!                         as a 6-character literal string. --- OBSOLETE
!   language            - Fortran
!                         DEC
!
! ----------------------------------------------------------------------
...
!
!     print error message
!     disable undefined error output (it's not an error)
!
      if (ier1 .eq. 4)  return
!
...
!                       - single/h36,h48,h60
!
!   reqd. IMSL routines - uertst1,ugetio1
!
!   notation            - information on special notation and
!                           conventions is available in the manual
!                           introduction or through IMSL routine uhelp
...
!   IMSL ROUTINE NAME   - IBCIEU  - Modified for real*8
!
!-----------------------------------------------------------------------
!
!   COMPUTER            - CRAY/SINGLE
...
!
!   USAGE               - CALL IBCIEU (F,IFD,X,NX,Y,NY,XL,NXL,YL,NYL,
!                           FL,IFLD,WK,IER)
!
!   ARGUMENTS    F      - NX BY NY MATRIX CONTAINING THE FUNCTION
!                           VALUES. (INPUT) F(I,J) IS THE FUNCTION VALUE
!                           AT THE POINT (X(I),Y(J)) FOR I=1,...,NX AND
!                           J=1,...,NY.
!                IFD    - ROW DIMENSION OF THE MATRIX F EXACTLY AS
!                           SPECIFIED IN THE DIMENSION STATEMENT
!                           IN THE CALLING PROGRAM. (INPUT)
!                X      - VECTOR OF LENGTH NX. (INPUT) X MUST BE
!                           ORDERED SO THAT X(I) .LT. X(I+1) FOR
!                           I=1,...,NX-1.
!                NX     - NUMBER OF ELEMENTS IN X. (INPUT) NX MUST BE
!                           .GE. 2.
!                Y      - VECTOR OF LENGTH NY. (INPUT) Y MUST BE
!                           ORDERED SO THAT Y(J) .LT. Y(J+1) FOR
!                           J=1,...,NY-1.
!                NY     - NUMBER OF ELEMENTS IN Y. (INPUT) NY MUST BE
!                           .GE. 2.
!                         NOTE - THE COORDINATE PAIRS (X(I),Y(J)), FOR
!                           I=1,...,NX AND J=1,...,NY, GIVE THE POINTS
!                           WHERE THE FUNCTION VALUES F(I,J) ARE
!                           DEFINED.
!                XL     - VECTOR OF LENGTH NXL. (INPUT)
!                NXL    - NUMBER OF ELEMENTS IN XL. (INPUT)
!                YL     - VECTOR OF LENGTH NYL. (INPUT)
!                NYL    - NUMBER OF ELEMENTS IN YL. (INPUT)
!                         NOTE - THE COORDINATE PAIRS (XL(I),YL(J)),
!                           FOR I=1,...,NXL AND J=1,...,NYL, GIVE THE
!                           POINTS AT WHICH THE INTERPOLATORY BICUBIC
!                           SPLINE IS TO BE EVALUATED.
!                FL     - NXL BY NYL MATRIX CONTAINING THE INTERPOLATORY
!                           BICUBIC SPLINE VALUES. (OUTPUT) FL(I,J) IS
!                           SET TO THE VALUE OF THE INTERPOLATORY
!                           BICUBIC SPLINE AT (XL(I),YL(J)) FOR
!                           I=1,...,NXL AND J=1,...,NYL. NOTE THAT THE
!                           NUMBER OF COLUMNS IN FL MUST BE .GE.
!                           MAX(NYL,NY) SINCE FL IS ALSO USED AS
!                           WORKING STORAGE (OF SIZE NXL BY NY) DURING
!                           THE COMPUTATION.
!                IFLD   - ROW DIMENSION OF THE MATRIX FL EXACTLY AS
!                           SPECIFIED IN THE DIMENSION STATEMENT
!                           IN THE CALLING PROGRAM. (INPUT)
!                WK     - WORK VECTOR OF LENGTH
!                           MAX((NX-1)*3,(NY-1)*3+NY).
!                IER    - ERROR PARAMETER. (OUTPUT)
!                         TERMINAL ERROR
!                           IER = 129, IFD IS LESS THAN NX.
!                           IER = 130, IFLD IS LESS THAN NXL.
!                           IER = 131, NX IS LESS THAN 2.
!                           IER = 132, NY IS LESS THAN 2.
!                           IER = 133, VECTOR X IS NOT ORDERED SO THAT
!                             X(1) .LT. X(2) ... .LT. X(NX).
!                           IER = 134, VECTOR Y IS NOT ORDERED SO THAT
!                             Y(1) .LT. Y(2) ... .LT. Y(NY).
!                         WARNING ERROR
!                           IER = 37, XL(I) IS LESS THAN X(1) OR
!                             GREATER THAN X(NX).
!                           IER = 38, YL(I) IS LESS THAN Y(1) OR
!                             GREATER THAN Y(NY).
!
!   PRECISION/HARDWARE  - SINGLE AND DOUBLE/H32
!                       - SINGLE/H36,H48,H60
!
!   REQD. IMSL ROUTINES - ICSEVU1,ICSCCU1,UERSET1,UERTST1,UGETIO1
!
!   NOTATION            - INFORMATION ON SPECIAL NOTATION AND
!                           CONVENTIONS IS AVAILABLE IN THE MANUAL
!                           INTRODUCTION OR THROUGH IMSL ROUTINE UHELP
...
!                                  CHECK NY .GE. 2
      IF (NY .LT. 2) GO TO 37
!                                  CALL UERSET1 TO SILENCE WARNING
!                                    MESSAGES FROM ICSEVU
      LEVEL = 2
...
!                OPERATIONS CALLS UGETIO1 TO OBTAIN THE CURRENT UNIT
!                IDENTIFIER VALUES. IF UGETIO1 IS CALLED WITH IOPT=2 OR
...
!                                  FIRST EXECUTABLE STATEMENT
   ...
   10 NOUTD = NOUT
 9005 RETURN
      END SUBROUTINE UGETIO1
!
!
!***** SUBROUTINE UERSET1 (LEVEL,LEVOLD)
!   IMSL ROUTINE NAME   - UERSET1
!
!-----------------------------------------------------------------------
!
!   COMPUTER            - CRAY/SINGLE
...
!   LATEST REVISION     - JANUARY 1, 1978
!
!   PURPOSE             - SET MESSAGE LEVEL FOR IMSL ROUTINE UERTST1
!
!   USAGE               - CALL UERSET1 (LEVEL,LEVOLD)
!
!   ARGUMENTS    LEVEL  - NEW VALUE FOR MESSAGE LEVEL. (INPUT)
!                           OUTPUT FROM IMSL ROUTINE UERTST1 IS
...
!     subroutine icsicu1 (x, y, nx, bpar, c, ic, ier)
! --- modified version of IMSL subroutine named ICSICU (REAL->REAL*8)

! called from:
! frsubs.f(1922):  CALL ICSCCU1 (X,F(1,IY),NX,WK(1),NXM1,JER)
! frsubs.f(1938):  CALL ICSCCU1 (Y,WK(KYLP1),NY,WK(1),NYM1,JER)

!----------------------------------------------------------------------
!   IMSL ROUTINE NAME   - ICSCCU
!   CUBIC SPLINE INTERPOLATION (EASY-TO-USE VERSION)
!-----------------------------------------------------------------------
!
!   COMPUTER            - VAX/SINGLE
!
...
!                           (OR FUNCTION VALUES) OF THE NX DATA POINTS.
!                           (INPUT)
!                NX     - NUMBER OF ELEMENTS IN X AND Y. (INPUT) NX     !210
!                           MUST BE .GE. 2.                             !220
!                C      - SPLINE COEFFICIENTS. (OUTPUT) C IS AN NX-1 BY !230
!                           3 MATRIX. THE VALUE OF THE SPLINE           !240
!                           APPROXIMATION AT T IS                       !250
!                           S(T) = ((C(I,3)*D+C(I,2))*D+C(I,1))*D+Y(I)  !260
!                           WHERE X(I) .LE. T .LT. X(I+1) AND           !270
!                           D = T-X(I).                                 !280
!                IC     - ROW DIMENSION OF MATRIX C EXACTLY AS          !290
!                           SPECIFIED IN THE DIMENSION STATEMENT IN     !300
!                           THE CALLING PROGRAM. (INPUT)                !310
!                IER    - ERROR PARAMETER. (OUTPUT)                     !320
!                         TERMINAL ERROR                                !330
!                           IER = 129, IC IS LESS THAN NX-1.            !340
!                           IER = 130, NX IS LESS THAN 2.               !350
!                           IER = 131, INPUT ABSCISSA ARE NOT ORDERED   !360
!                             SO THAT X(1) .LT. X(2) ... .LT. X(NX).    !370
!                                                                       !380
!   PRECISION/HARDWARE  - SINGLE AND DOUBLE/H32                         !390
!                       - SINGLE/H36,H48,H60                            !400
!                                                                       !410
!   REQD. IMSL ROUTINES - UERTST,UGETIO                                 !420
!                                                                       !430
!   NOTATION            - INFORMATION ON SPECIAL NOTATION AND           !440
!                           CONVENTIONS IS AVAILABLE IN THE MANUAL      !450
!                           INTRODUCTION OR THROUGH IMSL ROUTINE UHELP  !460
!                                                                       !470
!   COPYRIGHT           - 1980 BY IMSL, INC. ALL RIGHTS RESERVED.       !480
!                                                                       !490
!   WARRANTY            - IMSL WARRANTS ONLY THAT IMSL TESTING HAS BEEN !500
!                           APPLIED TO THIS CODE. NO OTHER WARRANTY,    !510
!                           EXPRESSED OR IMPLIED, IS APPLICABLE.        !520
!                                                                       !530
!-----------------------------------------------------------------------!540
!
      SUBROUTINE ICSCCU1 (X,Y,NX,C,IC,IER)
!                                  SPECIFICATIONS FOR ARGUMENTS         !570
      INTEGER            NX,IC,IER                                      !580
      REAL*8             X(NX),Y(NX),C(IC,3) !-YuP: changed REAL->REAL*8
!                                  SPECIFICATIONS FOR LOCAL VARIABLES   !600
      INTEGER            IM1,I,JJ,J,MM1,MP1,M,NM1,NM2                   !610
      REAL*8             DIVDF1,DIVDF3,DTAU,G,CNX(3) !-YuP: changed REAL->REAL*8
!                                  FIRST EXECUTABLE STATEMENT           !630
      NM1 = NX-1                                                        !640
      IER = 129                                                         !650
...
!                                  COMPUTE NOT-A-KNOT SPLINE            !710
 ...
 9005 RETURN                                                            !1420
      END SUBROUTINE ICSCCU1                                            



!---IMSL---UERTST------------------------------------------------IMSL---UERTST--
...
!                           EXPRESSED OR IMPLIED, IS APPLICABLE.
!-----------------------------------------------------------------------
      SUBROUTINE UERTST (IER) !,NAME)  YuP[2019-06-18] drop "name" - was only for printout
!                                  SPECIFICATIONS FOR ARGUMENTS
      INTEGER            IER
      INTEGER*2          NAME(3)
!                                  SPECIFICATIONS FOR LOCAL VARIABLES
...
!                                  FIRST EXECUTABLE STATEMENT
      ...
      data NAME/0,0,0/ !YuP[2019-06-18]: just for initialization
