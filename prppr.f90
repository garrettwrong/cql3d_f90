...
!mnt  and linearly interpolates to a quantity fpn in xpar,xperp
!mnt  coordinates.
!...............................................................
...
!     jpxy refers to xpar (make it odd).

!bh960801      if (mod(jpxy,2).eq.0) jpxy=jpxy-1
      ipxym=ipxy-1
      jpxym=jpxy-1
      icase=1
...
!990131        xminn=amin1(xmgrll,xmgrlu,xmgrul,xmgruu)
!990131        xmaxx=amax1(xmgrll,xmgrlu,xmgrul,xmgruu)
        xminn=min(xmgrll,xmgrlu,xmgrul,xmgruu)
        xmaxx=max(xmgrll,xmgrlu,xmgrul,xmgruu)
        if (icase.gt.1) xmaxx=1.
      else
!990131        xminn=amin1(xmgll,xmglu,xmgul,xmguu)
!990131        xmaxx=amax1(xmgll,xmglu,xmgul,xmguu)
      ...
      thul=acos(cossul)
      thuu=acos(cossuu)
!990131      thminn=amin1(thll,thlu,thul,thuu)
!990131      thmaxx=amax1(thll,thlu,thul,thuu)
 ...
 40   continue
      ...
      if (xlu.eq.(-xll)) then
!     Keeping xpar exactly antisymmetric, attempting to avoid minor
!     difficulties in unsymmetric interpolation near the v-grid edge..
         do 50 jp=2,jpxym/2
...
!      do jp=1,jpxy
!      write(*,*)'prppr: jp, xpar(jp)=',jp, xpar(jp)
!      enddo
!      do ip=1,ipxy
!      write(*,*)'prppr: ip, xperp(ip)=',ip, xperp(ip)
!      enddo
      ...
      do 130 ip=1,ipxy-1
        ...
        do 120 jp=2,jpxy-1
          ...
          cosse=xpar(jp)/xmg
!990131          if (abs(cosse).gt.1.)  cosse=sign(1.,cosse)
...
!      do jp=1,jpxy
!      do ip=1,ipxy
!         write(*,*)'prppr: jp,ip,fpn(jp,ip):',jp,ip,fpn(jp,ip)
!      enddo
!      enddo
!-obsolete      wmax=0.
!-obsoletec     (The first half-interval is integrated with average(fpn)*x_perp.
!-obsoletec      Thereafter, trapazoidal integration).
!-obsolete      do 150 jp=1,jpxy
!-obsolete        s=(3.*fpn(jp,1)+fpn(jp,2))*dxpp**2/32.
!-obsolete        do 140 ip=2,ipxym
!-obsolete          s=s+fpn(jp,ip)*xperp(ip)*dxpp
!-obsolete 140    continue
!-obsolete        s=s+fpn(jp,ipxy)*xperp(ipxy)*dxpp*.5
!-obsolete        fll(jp)=s*2.*pi
!-obsolete 150  continue
!-obsoletec     if (target.eq."mainmesh") then
!-obsoletec     do 155 jp=2,jpxy
!-obsoletec     dfvlle(jp)=(fpn(jp)-fpn(jp-1))*dvlli
!-obsoletec     155   continue
!-obsoletec     elseif (target.eq."ionmesh") then
!-obsoletec     do 156 jp=2,jpxy
!-obsoletec     dfvlli(jp)=(fpn(jp)-fpn(jp-1))*dvlli
!-obsoletec     156   continue
!-obsoletec     endif
!-obsolete      if (action.eq."norm") then
!-obsolete        call aminmx(fll(1),1,jpxy,1,fmin,fmax,kmin,kmax)
!-obsolete        do 160 jp=1,jpxy
!-obsolete          fll(jp)=fll(jp)/fmax
