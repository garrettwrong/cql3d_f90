...
!..................................................................
        ...
        roveramp=roveram
        hrz=(1.d0-roveramp)/(setup0%lrzmax-1)
        hrz1=hrz*rfacz
        rrz(0)=0.
        rrz(1)=roveramp
        rrz(2)=rrz(1)+hrz1
        rrmax= 1.d0-roveramp
        call micgetr(setup0%lrzmax,rrmax,hrz1,ram,ksingul)
        do 2 ll=3,setup0%lrzmax
...
!..................................................................
        ...
        rrz(1)=hrz1
        rrmax=one
        call micgetr(setup0%lrzmax+1,rrmax,hrz1,ram,ksingul)
        if (ksingul.eq.1) stop 'tdxinitl: problem with radial mesh'
        do 1 ll=2,setup0%lrzmax
          rrz(ll)=rrz(ll-1)+ram*(rrz(ll-1)-rrz(ll-2))
 1      continue
 ...
 6    continue
      ...
      ! Example: For roveram=1.e-8, setup0%lrz=50, rfacz=1.0 (rzset='disabled')
...
!     Interpolate input ion densities onto rya grid
         if (iprone.eq."parabola")  then
            ...
            do kk=1,nionm
               k=kionm(kk)
               call tdxin13d(reden,rya,setup0%lrzmax,ntotala,k,npwr(0),mpwr(0))
!               write(*,*)'tdxinitl: k,reden(k,1:setup0%lrzmax)=',
!     +                              k,(reden(k,i),i=1,setup0%lrzmax)
            enddo
...
!      write(*,*)'reden(kionm(1),ll)= ',(reden(kionm(1),ll),ll=0,setup0%lrzmax)
!      write(*,*)'reden(kionm(2),ll)= ',(reden(kionm(2),ll),ll=0,setup0%lrzmax)
...
!      write(*,*)'reden(kionm(1),ll)= ',(reden(kionm(1),ll),ll=1,setup0%lrzmax)
!      write(*,*)'reden(kionm(2),ll)= ',(reden(kionm(2),ll),ll=1,setup0%lrzmax)
...
!      write(*,*)'reden(kionm(2),ll)= ',(reden(kionm(2),ll),ll=1,setup0%lrzmax)
!      if (nionm.gt.2) then
!      write(*,*)'reden(kionm(3),ll)= ',(reden(kionm(3),ll),ll=1,setup0%lrzmax)
!      endif
...
!      do k=1,ngen
!         if (k.eq.kelecg) then
!         write(*,*)'reden(kelecg,ll)= ',(reden(kelecg,ll),ll=1,setup0%lrzmax)
!         endif
!         if (k.eq.kiongg(k)) then
!         write(*,*)'reden(kiongg(k),ll)= ',
!     +        (reden(kiongg(k),ll),ll=1,setup0%lrzmax)
!         endif
!      enddo
      ...
      if (iproti.eq."parabola")  then
         do 25  k=1,ntotal
            if (bnumb(k).ne.-1.) call tdxin13d( &
                 temp,rya,setup0%lrzmax,ntotala,k,npwr(k),mpwr(k))
 25      continue
...
!     vphipl, toroidal plasma velocity for use in freya
      call bcast(vphipl,zero,setup0%lrzmax)
...
!     tauegyz, eparc, eperc   ! YuP: tauegyz or tauegy?
      do 3 k=1,ngen
         if(tauegy(k,0).gt.0.) & ! YuP-101220: changed tauegyz to tauegy !
              call tdxin13d(tauegy,rya,setup0%lrzmax,ngena,k,negy(k),megy(k))
         if(torloss(k).ne."disabled")  then
            call tdxin13d(eparc,rya,setup0%lrzmax,ngena,k,ntorloss(k), &
                 mtorloss(k))
            call tdxin13d(eperc,rya,setup0%lrzmax,ngena,k,ntorloss(k), &
                 mtorloss(k))
         endif
...
!     arrays for source term
         nga=ngena
         do 4 kk=1,nso
            call tdxin23d(sellm1z,rya,setup0%lrzmax,nga,nsoa,k,kk,DBLE(npwrsou(k)), &
                 DBLE(mpwrsou(k)))
            call tdxin23d(sellm2z,rya,setup0%lrzmax,nga,nsoa,k,kk,DBLE(npwrsou(k)), &
                 DBLE(mpwrsou(k)))
            call tdxin23d(szm1z,rya,setup0%lrzmax,nga,nsoa,k,kk,DBLE(npwrsou(k)), &
                 DBLE(mpwrsou(k)))
            call tdxin23d(seppm1z,rya,setup0%lrzmax,nga,nsoa,k,kk,DBLE(npwrsou(k)), &
                 DBLE(mpwrsou(k)))
            call tdxin23d(seppm2z,rya,setup0%lrzmax,nga,nsoa,k,kk,DBLE(npwrsou(k)), &
                 DBLE(mpwrsou(k)))
            call tdxin23d(szm2z,rya,setup0%lrzmax,nga,nsoa,k,kk,DBLE(npwrsou(k)), &
                 DBLE(mpwrsou(k)))
            call tdxin23d(sem1z,rya,setup0%lrzmax,nga,nsoa,k,kk,DBLE(npwrsou(k)), &
                 DBLE(mpwrsou(k)))
            call tdxin23d(sem2z,rya,setup0%lrzmax,nga,nsoa,k,kk,DBLE(npwrsou(k)), &
                 DBLE(mpwrsou(k)))
            call tdxin23d(sthm1z,rya,setup0%lrzmax,nga,nsoa,k,kk,DBLE(npwrsou(k)), &
                 DBLE(mpwrsou(k)))
            call tdxin23d(scm2z,rya,setup0%lrzmax,nga,nsoa,k,kk,DBLE(npwrsou(k)), &
                 DBLE(mpwrsou(k)))

!     Allow for possiblity to intialize asorz(k,kk,1:setup0%lrzmax) directly
            if(asorz(k,kk,0).ne.-1.) then
               call tdxin23d(asorz,rya,setup0%lrzmax,nga,nsoa,k,kk,DBLE(npwrsou(0)), &
                    DBLE(mpwrsou(0)))
            endif
...
!     electric field profile
      if (eqsource.ne."tsc" .and. &
          .not.(ampfmod.eq."enabled" .and. setup0%nlrestrt.ne."disabled")) then
         ...
         if (iproelec.eq."parabola") then
            elecfldc=elecfld(0)
            elecfldb=elecfld(1)
            call tdxin33d(elecfld,rya,setup0%lrzmax,npwrelec,mpwrelec)
