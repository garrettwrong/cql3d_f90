...
CPGPLT      CALL PGSWIN(Rtam1(1),Rtam1(nen),Remin,Remax)
CPGPLT      CALL PGBOX('BCNST',0.,0,'BCNSTL',0.,0)
CPGPLT      CALL PGSAVE
CPGPLT      CALL PGSCH(1.44)
CPGPLT      if (softxry.eq.'enabled') then
CPGPLT      CALL PGLAB('Photon Energy k (keV)', 
CPGPLT     +     'd\u2\d\(0555)/dtdk (ergs/cm\u2\d-sec-ster-eV)',
CPGPLT     +     'SXR Energy Flux versus Photon Energy')
CPGPLT      else
CPGPLT      CALL PGLAB('Particle Energy k (keV)', 
CPGPLT     +     'd\u2\dN/dtdk (#/cm\u2\d-sec-ster-eV)',
CPGPLT     +     'NPA Flux versus Energy')
CPGPLT      endif
...
CPGPLT        CALL PGLINE(inegsxr(nn),RTAM1,RTAM2)
 200  continue ! nn
 
CPGPLT       CALL PGSLW(lnwidth) ! restore line thickness/width
...
CPGPLT      CALL PGMTXT('B',7.,-0.1,0.,t_)
...
CPGPLT         CALL PGMTXT('B',7.5+0.5*nn,0.,0.,t_)
 300  continue
c$$$      do 300  nn=1,nv,4
...
CPGPLT     +        'Flux Surfaces and SXR Chords')
CPGPLT      else  ! NPA
CPGPLT         CALL PGLAB('Major radius (cms)','Vert height (cms)',
CPGPLT     +        'Flux Surfaces and NPA Chords')
CPGPLT      endif
...
CPGPLT        CALL PGLINE(lensxr(nn),RRTAB1,RRTAB2)
