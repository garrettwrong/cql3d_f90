...
module cqcomm_mod
  ...
  common /diskx/ &
       ...
       rovera(lrza), &
       zmax(0:lrza), &
       lrindx(0:lrorsa),lsindx(0:lrorsa)
  ...
  common /diskx/ &
       reden(ntotala,0:lrza), &
       temp(ntotala,0:lrza)

  common /diskx/ &
       tauegy(ngena,0:lrza),eparc(ngena,0:lrza), &
  ...
  common /diskx/ &
       eegy(negyrga,2,ngena,lrza), &
       jegy(negyrga,2,ngena,lrza)
  ...
  common /diskx/ &
       mpwrsou(0:ngena),npwrsou(0:ngena)

  common /diskx/ &
       asor(ngena,nsoa,lrza)
  ...
  common /readscal/ &
       ...
       pltso, &
       soucoord, &
       knockon,komodel,nkorfn,nonko,noffko,soffvte,soffpr, &
       flemodel,jfl,xlfac,xlpctlwr,xlpctmdl,xllwr,xlmdl

  common /readarr/ &
       nonso(ngena,nsoa),noffso(ngena,nsoa), &
       sellm1(ngena,nsoa),sellm2(ngena,nsoa),seppm1(ngena,nsoa), &
  ...
  character*8 &
       urfmod, &
       vlfmod,vlfbes,vlfnpvar,vlhmod,vprprop,vlhplse,vlhprprp, &
  ...
  common/readscal/ urfmod, &
       vlhmod,vlhmodes,vprprop,vdalp,vlh_karney, &
       vlhplse,vlhpon,vlhpoff, &
       vlfmod,vlfmodes, &
       vlfbes,vlfnpvar,rdc_upar_sign, &
       rfread,rdcmod,rdc_clipping,rdc_netcdf

  common /readvec/ &
       nonrf(ngena),noffrf(ngena), &
       dlndau(nmodsa),vparmin(nmodsa),vparmax(nmodsa), &
       vlhprprp(nmodsa),vprpmin(nmodsa),vprpmax(nmodsa), &
       vlhpolmn(nmodsa),vlhpolmx(nmodsa), &
       vlffreq(nmodsa),vlfharms(nmodsa),vlfharm1(nmodsa), &
       vlfnp(nmodsa),vlfdnp(nmodsa),vlfddnp(nmodsa), &
       vlfeplus(nmodsa),vlfemin(nmodsa), &
  ...
  common/arr3d/ &
       acoefne(4),acoefte(4),ryain(njenea),elecin(njenea), &
       enein(njenea,ntotala),tein(njenea),tiin(njenea), &
       ennin(njenea,npaproca),irzplt(lrorsa), &
       rya(0:lrza+1), &
       thet1(nva),thet2(nva),thet1_npa(nva),thet2_npa(nva), &
       nplt3d(nplota)

  common/arr3d/ &
       enein_t(njenea,ntotala,nbctimea),tein_t(njenea,nbctimea), &
  ...
  common/arr3d/ &
       sellm1z(ngena,nsoa,0:lrza),sellm2z(ngena,nsoa,0:lrza), &
  ...
  character*8 &
       ...
       transp,adimeth, &
       efswtch,efswtchn,efiter,efflag,npa_process

  character*256 difus_io_file

  common /s3d/ &
       difusr,advectr,difus_io_file, &
       enmin,enmax,bootst,bootcalc,bootupdt,bootsign,pinch, &
       fds, &
       iprone,iprote,iproti,iprozeff,iprovphi,iproelec,ipronn,iprocur, &
       tmdmeth,kfrsou, &
       ...
       plt3d,pltvs,partner,radcoord, &
       rfacz,rzset,roveram,relaxden,relaxtsp, &
       ndeltarho,softxry,npa_diag, &
       transp, &
       enmin_npa,enmax_npa,fds_npa, &
       atten_npa,adimeth,nonadi, &
       efswtch,efswtchn,efiter,efflag, &
       curr_edge,efrelax,efrelax1,currerr

  common/ar3d/ &
       difus_rshape(8),difus_vshape(4),difin(njenea), &
       rd(nva),thetd(nva),x_sxr(nva),z_sxr(nva), &
       rd_npa(nva),thetd_npa(nva),x_npa(nva),z_npa(nva), &
       npa_process(npaproca), &
       difus_io_drrscale(nbctimea,ngena),  &
  ...
  character*8 &
       eqsym,eqdskalt,eqsource,eqmodel, &
       fpsimodl
  ...
  common/readscal/ &
       atol, &
       ellptcty,eqmodel,eqpower,eqsource,eqdskin,bsign, &
       eqsym,eqdskalt, &
       fpsimodl, &
       methflag, &
       povdelp, &
  ...
  common/readvec/ &
       pwrscale(nmodsa),wdscale(nmodsa),nrfstep1(nmodsa), &
       nharms(nmodsa),nharm1(nmodsa),nrfspecies(nmodsa), &
       iurfcoll(nmodsa),iurfl(nmodsa),rftype(nmodsa)

  common/readvec/ &
       pwrscale1(nbctimea),urftime(nbctimea)

  common/readvec/ &
       rdcscale(nrdca),nrdcspecies(nrdca)
  ...
  common /readscal/ &
       ...
       epsthet, &
       elpar0, &
       lmidpln,lmidvel,laddbnd, &
       nchgdy,ngauss,nlagran, &
       nonavgf,nofavgf, &
       nontran, nofftran, nonelpr, noffelpr, &
       nlrestrt,nlwritf,nummods,numixts, &
       oldiag, &
       sbdry,scheck,ampfmod,eseswtch, &
       updown,ampferr,nampfmax,nonampf

  common /readvec/ &
       denpar(ntotala,0:lza+1), &
       nkconro(ntotala),nlotp1(noutpta),nlotp2(noutpta), &
       nlotp3(noutpta),nlotp4(noutpta), &
       temppar(ntotala,0:lza+1)
  ...
  common /params/ &
       idim,iyjx,iyjxp1,iyp1jx, iyjx2, iyp1,jxp1, &
       lrors, &
       mxp1,mbet, &
  ...
  !
  common /diskx/ &
       bmod0(lrza),btor0(lrza),bthr(lrza),btoru(lrza),bthr0(lrza), &
  ...
  common /diskx/ &
       taubi(lrza),tau_neo_b(lrza),drr_gs_b(lrza), &
  ...
  common /diskx/ &
       ...
       ucrit(ngena,lrza),jxcrit(ngena,lrza),      &
       jchang(ngena,lrorsa), &
       denra(ngena,lrza),curra(ngena,lrza), &
  ...
  common &
       ipad1,r0geomp, rgeomp, zgeomp, rgeom1, rgeom2, &
       niyjx,nccoef,ncentr,navnc,ncplt,n, &
       nflux,nframe,npltmx, &
       outfile,ipad2,one,one_, &
       prefix, &
       pi,pio180,pio2,psir,proton, &
       rgyro,resist,resistn,rovsf,rmp0,rovscf,rbgn, &
       stopm, &
       tei,timet,t_,three,third,sevenhun,two,twopi, &
       vparl,vpovth, &
       xconn,xmax,ipxy,jpxy, &
       zero,zstar, &
       iplot,nplott,isave,nsavet,nirzplt, &
       nefiter ! counts iterations for el.field (efswtch="method5")
  common &
       elecfldc,elecfldb,  & !V/cm, Added for ampfmod case
  ...
  !BH111124     1  iota(0:750),realiota(0:750), &
       iota(0:6400),realiota(0:6400), &
       i1p(2),itab(3),ipad4,tab(3), &
       imsh(20), &
       frbuf(1024), &
       ws2d(2500), &
       work(tlfld1a)
  ...
  common &
       gamt(ntotala,ntotala),gama(ntotala,ntotala), &
  ...
  common /diskx/ &
       bdre(lrza),bdrep(lrza), &
       sorpwt(lrza),sorpwti(0:lrza), &
       sorpw_nbii(1:ngena,0:lrza),sorpw_rfi(1:ngena,0:lrza), &
       xlncurt(lrza)

  common /diskx/ &
       sorpw_rf(1:ngena,lrza),sorpw_nbi(1:ngena,lrza)

  common /diskx/ &
       cosm1(ngena,nsoa,lrza),cosm2(ngena,nsoa,lrza), &
  ...
  real*8 li
  common/sc3d/ &
       dttr,dtreff,currtza,currtpza,conserv, &
  ...
  common/ar3d/ rrz(0:lrza), &
       ...
       jparb(lrza),jparbt(lrza),jparbp(lrza), &
       prestp(lrza),prest(lrza),d2prest(lrza),d2fpsiz(lrza), &
       d2ffpsiz(lrza), &
  ...
  common /csxr/ sxry(lrza,4),sang(lrza,4),spol(lrza,4), &
       ...
       enk(nena),en_(nena),jval_(nena), &
       inegsxr(nva),lensxr(nva)

  common /csigma/ mtab,msig,jxis,elmin,delegy, &
       imaxwln(2,4),igenrl(2,4), &
       sigm(4,lrorsa),sigf(4,lrorsa),sigmt(4),sigft(4), &
  ...
  common /diskx/ &
       ...
       bpsi_max(lrza),bpsi_min(lrza),lbpsi_max(lrza),lbpsi_min(lrza), &
       z_bmax(lrza),bpsi_z_bmax(lrza),lz_bmax(lrza), &
  ...
  common &
       ibd(4), &
       eqpsi(nconteqa),eqvol(nconteqa),eqfopsi(nconteqa), &
  ...
  common &
       epsi(nnra,nnza),epsirr(nnra,nnza), &
  ...
  common/output/ lorbit_,ialign14,rmcon_,rpcon_,zmcon_,zpcon_, &
       ...
       es_bmax_,bpsi_max_,bpsi_min_,lbpsi_max_,lbpsi_min_, &
       bmidplne_,solr_(lfielda),solz_(lfielda),es_(lfielda), &
  ...
  common &
       bsslstp(nmodsa), &
       ncontrib(lrza), &
       powrf(lrza,nmodsa),powrfc(lrza,nmodsa), &
       powrfl(lrza,nmodsa),powrft(lrza), &
       powurf(0:nmodsa),powurfc(0:nmodsa),powurfl(0:nmodsa), &
       powurfi(0:lrza,0:nmodsa)

  common &
       freqcy(nmodsa),omega(nmodsa),nharm(nmodsa),nray(nmodsa), &
       bsign1(nmodsa),krfn(nmodsa),irfn(nmodsa),irfm(nmodsa), &
       irffile(nmodsa)
  ...
  common anecc(nrada),tekev(nrada),tikev(nint1a,nrada), &
       ...
       nspc,ialign9,fpsiar_(nrada),pary(nrada),ppary(nrada), &
       gpary(nrada),ztop_,zbot_,rleft,rright,nx,nz, &
       npsitm
  ...
  common /wpscal/ &
       analegco, &
       iymax, &
       nsleft,nsrigt,numclas,numindx

  common /wpvec/ &
       cofdfds(0:lsa1,2,ntrmdera), &
       enrgypa(ntotala,0:lsa1), &
       vthpar(ntotala,0:lsa1), &
       lsbtopr(0:lsa1),lsprtob(0:lsa1),lpm1eff(0:lsa1,-1:+1), &
       sz(0:lsa1),dsz(0:lsa1),dszm5(0:lsa1),dszp5(0:lsa1), &
  ...
  common /freycomm/ &
       frmodp, fr_gyrop, beamplsep,beamponp,beampoffp, &
       hibrzp,mfm1p
