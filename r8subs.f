...
cBH140214: Downloaded dgesv.f and dependencies from Netlib, for use.
cBH140214: by ampfsoln.  dgesv.f solves set of linear equations.
cBH140214: Wrote simple test program (commented out below).      
cBH140214: Also, commented out dependencies which are already in r8subs.f.
cBH140214: (Had 1st tried dgesvxx which is more complicated, and as it
cBH140214: turned out, had an unsatisfied blas routine, so moved on.)

      
c$$$
c$$$      program simple_test_4_dgesv
c$$$
c$$$c     From http://www.netlib.org, search dgesv
c$$$c     gfortran -o simple_test_4_dgesv -g -fbounds-check -L/usr/lib64 -lblas simple_test_4_dgesv.f
c$$$c     gfortran -o simple_test_4_dgesv -g -fbounds-check r8subs.f simple_test_4_dgesv.f
c$$$c     Needs libblas for idamax,dswap,dscal,dger,dtrsm,dgemm.
c$$$c     Alternatively, these blas routines are in r8subs.f.
c$$$
c$$$      parameter(NN=2)
c$$$      real*8 a(NN,NN)
c$$$c      real*8 AF(2,2)  !Output for FACT='E'
c$$$c      real*8 R(2)
c$$$c      real*8 C(2)  !Output
c$$$c      real*8 B(2,1)  ! (LDB,NRHS)
c$$$      real*8 B(NN)  ! (LDB)
c$$$c      real*8 X(NN)
c$$$c      real*8 RCOND,RPVGRW
c$$$c      real*8 BERR(1)  ! dimension (NRHS)
c$$$c      real*8 ERR_BNDS_NORM(1,3),ERR_BNDS_COMP(1,3) !(NRHS, N_ERR_BNDS)
c$$$c      real*8 PARAMS(0)
c$$$c      real*8 WORK(8)   !(4*N)
c$$$      integer IPIV(NN)  !Output for FACT='E'
c$$$c      integer N,NRHS,LDA,LDAF,LDB,LDX,N_ERR_BNDS,NPARAMS,INFO
c$$$      integer N,NRHS,LDA,LDB,INFO
c$$$c      character*1 FACT,TRANS
c$$$c      character*1 EQUED   !Output for FACT='E' 
c$$$c     Check if any diff between 1 and 1d0, etc.
c$$$      a(1,1)=1d0
c$$$      a(1,2)=2d0
c$$$      a(2,1)=3d0
c$$$      a(2,2)=4d0
...
c$$$c      NPARAMS=0
c$$$
c$$$c$$$      call dgesvxx( FACT, TRANS, N, NRHS, A, LDA, AF, LDAF, IPIV,
c$$$c$$$     $                    EQUED, R, C, B, LDB, X, LDX, RCOND, RPVGRW,
c$$$c$$$     $                    BERR, N_ERR_BNDS, ERR_BNDS_NORM,
c$$$c$$$     $                    ERR_BNDS_COMP, NPARAMS, PARAMS, WORK, IWORK,
c$$$c$$$     $                    INFO )
c$$$
c$$$      call dgesv(  N, NRHS, A, LDA, IPIV,
c$$$     $                    B, LDB,
c$$$     $                    INFO )
c$$$
c$$$      write(*,*) 'INFO=', info, '   B_out=',b(1),b(2)
c$$$
c$$$      end
c$$$
